...
//!   cargo run -- snapshot [t] # one live snapshot of the running engine, right now
//!   cargo run -- stream [days]# emit the world + a stream of live engine snapshots (viewer/)
...
fn main() {
    ...
    match mode {
        ...
        "dog" => dog_view(),
        // The live engine: a stream of snapshots for the viewer, or one snapshot now.
        "stream" | "trace" => print!("{}", stream_json(args.get(1).and_then(|s| s.parse().ok()).unwrap_or(7))),
        "snapshot" => snapshot_view(args.get(1).and_then(|s| s.parse().ok()).unwrap_or(12)),
        "district" => print_district(&build_world()),
        _ => {
            eprintln!("unknown mode '{mode}'. try: normal | matchday | week | days N | explain NAME | chronicle | dog | stream [days] | snapshot [ticks] | district");
        }
    }
}
...
// ------------------------------------------------ the live engine (for the viewer)

/// Emit the world plus a stream of live engine snapshots for `days`.
///
/// This is no longer a bespoke "trace" hand-rolled in the observer — it is the
/// engine's own live-state interface. We hold one persistent `Engine`, tick it
/// hour by hour, and record what it reports at each instant. The renderer replays
/// these frames on a real-time clock and draws *only* what each snapshot says.
///
/// Shape: `{"world": <static stage>, "frames": [<snapshot per tick>, ...]}`.
/// The first frame is the world at the first breath (tick 0), before anything has
/// happened; each subsequent frame is the state after one more hour.
fn stream_json(days: u64) -> String {
...
/// Print a single live snapshot — proof that the engine can be observed at any
/// instant without replaying anything. `snapshot [ticks]` ticks the world that
/// many hours, then reports its state right now.
fn snapshot_view(ticks: u64) {
    let mut engine = Engine::new();
    engine.tick_n(ticks);
    ...
    for e in &snap.entities {
        let moving = if e.traveling { "  (moving)" } else { "" };
        println!("  {:<12} {:<20} {}{}", e.name, e.place_name, e.doing, moving);
    }
    if let Some((_, name, n)) = &snap.busiest {
        println!("\nBusiest place: {name} ({n} gathered)");
    }
    if !snap.callouts.is_empty() {
        println!("\nHappening now:");
        for c in &snap.callouts {
            println!("  · {}", c.text);
        }
    }
    println!(
        "\nThe Old Oak: {} yrs, {} this {} — {} visits, {} scarves, {} bouquets",
        snap.oak.age_years, snap.oak.appearance, snap.oak.season,
        snap.oak.visits, snap.oak.scarves, snap.oak.bouquets
    );
    if !snap.bonds.is_empty() {
        println!("\nStrongest bonds so far:");
        for b in snap.bonds.iter().take(4) {
            let place = b.usual_place.map(|p| format!(", usually at the {p}")).unwrap_or_default();
            println!("  · {} & {} — {} meetings (affinity {}){}", b.a, b.b, b.meetings, b.affinity, place);
        }
    }
}
